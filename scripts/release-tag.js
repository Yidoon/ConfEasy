...
if (!validTypes.includes(incrementType)) {
  console.error(
    `Error: Invalid increment type "${incrementType}". Must be one of: ${validTypes.join(
      ", "
    )}`
  )
  process.exit(1)
...
if (versionParts.length !== 3 || versionParts.some(isNaN)) {
  console.error(
    `Error: Invalid version format "${currentVersion}". Expected format: X.Y.Z`
  )
  process.exit(1)
}
...
const [newMajor, newMinor, newPatch] = calculateNewVersion(
  major,
  minor,
  patch,
  incrementType
